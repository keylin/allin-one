...
| `/g-develop` | sonnet | 编码(A/B模式) → 自动审查 → 自动修复 → 文档同步 | 功能开发实现 |
...
| `/mcp-deploy` | sonnet | MCP Server 更新部署（开发→验证→部署→重启） | `/g-ship` 后 MCP 变更时 |
...
| `doc-maintainer` | sonnet | /g-accept | 文档同步：评估变更影响→分级执行（有把握直接改，不确定才确认） |
...
| 更新/部署 MCP 工具 | `/mcp-deploy` | 手动 rsync + restart（容易忘重启） |
...
7. **文档同步多层防线**：CLAUDE.md 指令（行为内化）→ Stop hook（退出拦截 `scripts/utils/check-doc-drift.sh`）→ g-develop 阶段 4（主动更新）→ g-accept doc-maintainer（复核），确保文档不过期
