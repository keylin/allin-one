...
- **后台播放**: 播客音频由全局播放器统一承载，关闭详情面板/弹窗、切换条目、切换页面均不中断；视频在内嵌播放器销毁时自动将音轨交给全局播放器。底部迷你播放条提供进度条、暂停/播放、±15s、停止；支持锁屏续播与系统媒体控件（Media Session）
- **小窗播放**: 本地视频在内嵌播放器与迷你播放条上均可进入画中画（iOS 用 WebKit 接口，iOS standalone PWA 不支持时提示改用 Safari），小窗随全局播放器常驻，可边看边浏览其他内容
...
| `/content` | 信息站（侧栏分组「内容库」） | 表格管理、筛选排序去重 |
