...
数据源只描述「信息从哪来」，不涉及处理逻辑。处理方式由绑定的流水线模版决定。
...
- **大模型 API**: Provider 选择、API Key、Base URL、模型名称
- **任务并发度**: Worker 数量、并行任务上限
- **代理配置**: HTTP/HTTPS/SOCKS 代理
- **通知配置**: 邮件、钉钉等推送渠道
...
| `/pipelines` | 流程监控 | Pipeline 执行状态与日志 |
| `/video-download` | 视频下载 | 视频下载与播放 |
| `/prompt-templates` | 提示词管理 | LLM 分析模版配置 |
