...
>
> 进度：**P0 ✅**（清理只作用于网络采集类数据源 + 媒体收藏纳入保护；用户数据类数据源拒绝非级联删除）· **P1 ✅**（`content_items.kind` + 迁移 0025 回填；影片身份改按 kind 判定并加部分唯一索引；内容列表 / 未读 / 全部已读 / delete-all / 仪表盘 / 日报周报 / MCP 统一带 `FEED_SCOPE`；部署脚本改为先迁移后切换容器）· **P2 第 1–3 步 ✅**（源类型注册表 `app/models/source_types.py` 单点化，COLLECTOR_MAP / SYNC_FETCHERS 导入时对账；调度器 / 采集任务 / 采集端点 / 建源 / 改源 / 全量导入 / MCP 统一按注册表判定，不可采集的源返回 400 而不是写假成功记录；迁移 0026 清除 264 条假采集记录并归位调度状态；各领域同步端点校验领域；`run_sync` 僵死任务 15 分钟回收；Emby 每 30 分钟自动同步，影片数据无变化不写库；前端按 `collectable` / `runner` 决定显示「采集」还是「去同步」）。第 4 步改方案 ✅：不合表（入口拦截后两张表不再互相污染，且调度算法直接依赖 collection_records），只补外部推送的运行记录——三个推送端点经 `run_push_sync` 写 `sync_task_progress`（`_trigger=push`），成功/失败都留痕 · **P3 ✅（电子书与流水线相关项按决策跳过）**：删除死列 `content_items.language`（迁移 0027）与恒为空的响应字段；CollectionRecord 索引声明与库对齐；`/video` 与 `/media` 的播放进度端点合为一份实现；修复内容详情里不存在的 `/api/media/{id}/stream` 兜底地址；删除孤儿页面 VideoView；同步触发 + SSE 进度抽为 `useSyncRun` composable（SyncView / FilmsView 共用）；新增 `cleanup_job_queue`（每日清理 7 天前结束的作业、回收心跳丢失超 1 小时的僵死作业）。· **P4 ✅**：术语统一为「数据源」（界面 40 处、后端文案 21 处；「订阅源」专指 RSS feed，保留）；根 `CLAUDE.md` 的核心架构约束按「两条主线」重写；`system_design.md` §2 表结构改为**由模型生成**，§4.0 重写为四种执行器，API 清单 / cron（UTC）/ Collector 签名订正；两份整体过期的设计稿归档到 `docs/archive/`；术语表补执行器、未实现类型与术语约定；新增 `scripts/verify/drift/check.sh` 做「模型 vs 真实库」「文档 vs 模型」两项漂移检查——首次运行查出 23 处模型声明落后于库（19 个迁移建的索引未声明、2 处 nullable 不符等），已全部补齐（只改声明，无迁移、无行为变化）。
>
> **留待后续**（均不影响正确性）：错误码与接口命名统一（审计 §4 🔴-6，涉及前端 153 处判断）；去掉启动时的 `create_all`；`/video/downloads` 等已无前端调用方的端点；用户状态散布（收藏三处、进度四套）只定了清理保护的规矩，未做收敛；电子书域与 LLM 流水线按决策未动；日报 / 周报两个定时任务已于 2026-09-20 停用（根因不是「缺 Key」本身，而是 `get_llm_config()` 在没 Key 时 raise、破坏了 report_tasks 里「没 Key 就跳过 AI 总结」的降级路径；恢复前先修它）
