...
>
> 进度：**P0 ✅**（清理只作用于网络采集类数据源 + 媒体收藏纳入保护；用户数据类数据源拒绝非级联删除）· **P1 ✅**（`content_items.kind` + 迁移 0025 回填；影片身份改按 kind 判定并加部分唯一索引；内容列表 / 未读 / 全部已读 / delete-all / 仪表盘 / 日报周报 / MCP 统一带 `FEED_SCOPE`；部署脚本改为先迁移后切换容器）· **P2 第 1–3 步 ✅**（源类型注册表 `app/models/source_types.py` 单点化，COLLECTOR_MAP / SYNC_FETCHERS 导入时对账；调度器 / 采集任务 / 采集端点 / 建源 / 改源 / 全量导入 / MCP 统一按注册表判定，不可采集的源返回 400 而不是写假成功记录；迁移 0026 清除 264 条假采集记录并归位调度状态；各领域同步端点校验领域；`run_sync` 僵死任务 15 分钟回收；Emby 每 30 分钟自动同步，影片数据无变化不写库；前端按 `collectable` / `runner` 决定显示「采集」还是「去同步」）。第 4 步（运行记录合表）待定
