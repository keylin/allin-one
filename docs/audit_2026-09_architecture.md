...
>
> **留待后续**（均不影响正确性）：错误码与接口命名统一（审计 §4 🔴-6，涉及前端 153 处判断）；去掉启动时的 `create_all`；`/video/downloads` 等已无前端调用方的端点；用户状态散布（收藏三处、进度四套）只定了清理保护的规矩，未做收敛；电子书域与 LLM 流水线按决策未动，日报 / 周报仍因未配置 LLM Key 每日静默失败
