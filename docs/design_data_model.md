...
> 版本: v1.0 | 日期: 2026-09-23 | 状态: 方案定稿，待实施（实施顺序见 §9；决策记录与待定项在 the-one `project/allin-one/`）
...
三条前提（决策与理由见 the-one `project/allin-one/02-决策记录.md`）：**先构建完整视图、用法边建边探索**；**可独立离线运行**，落地资源皆为自有副本、无"缓存可重拉"一类；**allin-one 存记录，the-one 存判断**，the-one 引用只写 `content:<id>`，两边零共享存储。
...
阈值、`raw_data` 保留策略、状态词表位置、与既有定案的冲突，均记录在 the-one `project/allin-one/02-决策记录.md`，拍板后回填本文档对应章节。
