...
// Local media item favorited state (keyed by media item id)
const mediaFavorited = ref({})

async function handleMediaFavorite(mi) {
  const prev = mediaFavorited.value[mi.id] !== undefined ? mediaFavorited.value[mi.id] : mi.is_favorited
  mediaFavorited.value = { ...mediaFavorited.value, [mi.id]: !prev }
  try {
    const res = await toggleMediaFavorite(mi.id)
    if (res.code === 0) {
      mediaFavorited.value = { ...mediaFavorited.value, [mi.id]: res.data.is_favorited }
...
function isMediaFavorited(mi) {
  return mediaFavorited.value[mi.id] !== undefined ? mediaFavorited.value[mi.id] : (mi.is_favorited || false)
}
...
function handleContentDblClick(e) {
  ...
  const imageMediaItems = props.item?.media_items?.filter(m => m.media_type === 'image') || []

  const allImgs = Array.from(container.querySelectorAll('img'))
    .filter(el => el.src && !el.src.startsWith('data:'))
    .map(el => {
      const src = el.src
      // Try to match to a MediaItem by filename
      const filename = src.split('/').pop()
      const mi = imageMediaItems.find(m => m.local_path && m.local_path.split('/').pop() === filename)
      if (mi) {
        return { url: src, id: mi.id, isFavorited: isMediaFavorited(mi) }
      }
      return src
    })
  ...
  const idx = allImgs.findIndex(item => (typeof item === 'string' ? item : item.url) === clickedSrc)
...
<template>
  ...
  <div :class="isMobileOverlay ? 'flex flex-col min-h-[50vh]' : ''">
    ...
    <!-- 媒体项收藏条（多个 video/audio/image MediaItem 时显示） -->
    <div
      v-if="item.media_items?.filter(m => ['video','audio','image'].includes(m.media_type)).length > 0"
      :class="isMobileOverlay ? 'px-4 py-1' : 'py-1'"
    >
      <div class="flex flex-wrap gap-2">
        <div
          v-for="mi in item.media_items.filter(m => ['video','audio','image'].includes(m.media_type))"
          :key="mi.id"
          class="group inline-flex items-center gap-1.5 px-2 py-1 rounded-lg border text-xs transition-all"
          :class="isMediaFavorited(mi)
            ? 'bg-amber-50 border-amber-200 text-amber-700'
            : 'bg-slate-50 border-slate-200 text-slate-500 hover:border-slate-300'"
        >
          <!-- 图标 -->
          <svg v-if="mi.media_type === 'video'" class="w-3 h-3 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
          </svg>
          <svg v-else-if="mi.media_type === 'audio'" class="w-3 h-3 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M9 9l10.5-3m0 6.553v3.75a2.25 2.25 0 01-1.632 2.163l-1.32.377a1.803 1.803 0 11-.99-3.467l2.31-.66a2.25 2.25 0 001.632-2.163zm0 0V2.25L9 5.25v10.303m0 0v3.75a2.25 2.25 0 01-1.632 2.163l-1.32.377a1.803 1.803 0 01-.99-3.467l2.31-.66A2.25 2.25 0 009 15.553z" />
          </svg>
          <svg v-else class="w-3 h-3 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 001.5-1.5V6a1.5 1.5 0 00-1.5-1.5H3.75A1.5 1.5 0 002.25 6v12a1.5 1.5 0 001.5 1.5zm10.5-11.25h.008v.008h-.008V8.25zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z" />
          </svg>
          <!-- 状态 -->
          <span class="truncate max-w-[60px]">{{ mi.filename || mi.media_type }}</span>
          <span
            class="w-1.5 h-1.5 rounded-full shrink-0"
            :class="mi.status === 'downloaded' ? 'bg-emerald-400' : mi.status === 'failed' ? 'bg-rose-400' : 'bg-slate-300'"
          />
          <!-- 收藏按钮 -->
          <button
            class="shrink-0 transition-colors"
            :class="isMediaFavorited(mi) ? 'text-amber-500' : 'text-slate-300 hover:text-amber-400'"
            :title="isMediaFavorited(mi) ? '取消收藏' : '收藏此媒体'"
            @click.stop="handleMediaFavorite(mi)"
          >
            <svg class="w-3 h-3" :fill="isMediaFavorited(mi) ? 'currentColor' : 'none'" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" />
            </svg>
          </button>
        </div>
    ...
    <div
      ...
      :class="isMobileOverlay ? 'mx-4 my-2' : ''"
    >
      <button
        class="w-full flex items-center gap-2 px-3 py-2 text-xs font-medium text-slate-500 bg-slate-50 hover:bg-slate-100 rounded-lg border border-slate-200/80 transition-colors"
        @click="dupExpanded = !dupExpanded"
      >
        <svg
          class="w-3.5 h-3.5 text-slate-400 transition-transform duration-200"
          :class="dupExpanded ? 'rotate-90' : ''"
      ...
      >
        <div v-if="dupExpanded" class="mt-1.5 overflow-hidden">
          <div class="space-y-1">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              class="flex items-center gap-2 px-3 py-2 text-xs rounded-lg hover:bg-slate-50 border border-transparent hover:border-slate-200 transition-all group"
            >
              <span class="w-1.5 h-1.5 rounded-full shrink-0" :class="dup.is_original ? 'bg-indigo-400' : 'bg-slate-300'"></span>
              <span class="font-medium text-slate-600 truncate">{{ dup.source_name || '未知来源' }}</span>
              <span class="text-slate-300">&middot;</span>
              <span class="text-slate-400 shrink-0">{{ dup.collected_at ? formatTimeShort(dup.collected_at) : '' }}</span>
              <svg class="w-3 h-3 text-slate-300 group-hover:text-indigo-500 ml-auto shrink-0 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 6H5.25A2.25 2.25 0 003 8.25v10.5A2.25 2.25 0 005.25 21h10.5A2.25 2.25 0 0018 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25" />
              </svg>
            </a>
          </div>
