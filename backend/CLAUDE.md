...
- JSONB 字段（`raw_data`、`analysis_result`、`metadata_json`、`periodicity_data`）已迁移为 `Column(JSONB)`，直接作为 Python dict 读写，禁止再用 `json.loads`/`json.dumps`:
  ```python
  ...
  ```
- 仍为 Text 的字段（`config_json`、`system_settings.value` 等）继续用 `json.loads`/`json.dumps`
